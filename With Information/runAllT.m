...
for cpx_i=1:length(cpx)
    ...
    for cix_i=1:length(cix)
        c_i=cix(cix_i);
        [p, q, T, B_org, fval, flag_p, flag_q, flag_T] = runParallelHomogeneousIPT( n, M, c_p, c_i);
...
writetable(t,'ParallelHomogeneousWithInfomationProviderT.xlsx',"WriteMode","overwrite","AutoFitWidth",true);
end